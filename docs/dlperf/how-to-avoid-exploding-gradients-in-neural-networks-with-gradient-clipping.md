...
梯度爆炸问题的一个常见且相对简单的解决方案是，在通过网络反向传播误差并使用它来更新权重之前，改变误差的导数。两种方法包括给定选定的[向量范数](https://machinelearningmastery.com/vector-norms-machine-learning/)重新缩放梯度，以及剪切超过优选范围的梯度值。这些方法统称为“*渐变裁剪*”
...
一般来说，通过仔细配置网络模型，例如选择小的学习速率、缩放的目标变量和[标准损失函数](https://machinelearningmastery.com/how-to-choose-loss-functions-when-training-deep-learning-neural-networks/)，可以避免梯度爆炸。然而，对于具有大量输入时间步长的递归网络，梯度爆炸可能仍然是一个问题。
