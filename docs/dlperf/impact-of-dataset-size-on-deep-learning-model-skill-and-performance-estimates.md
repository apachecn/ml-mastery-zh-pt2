...
*   需要多少数据来合理地估计映射函数的近似值的性能？

一般来说，众所周知，训练数据太少会导致近似性差。过度约束的模型会使小的训练数据集不足，而欠约束的模型反过来可能会使训练数据过多，两者都会导致较差的性能。太少的测试数据将导致对模型性能的乐观和高方差估计。
...
尽管神经网络是一种高方差低偏差的方法，但这并不是在相同生成的数据点上运行相同算法时估计性能差异的原因。

相反，由于学习算法是随机的，我们在多次运行中看到了性能的差异。
...
这可能会令人沮丧，因为每次运行学习算法时，它经常会找到不同的解决方案，有时解决方案之间的差异很大，导致在对新数据进行预测时模型的估计性能存在差异。

### 平均模型性能

我们可以通过总结该方法在多次运行中的性能来抵消特定神经网络发现的解决方案中的差异。

这包括在同一数据集上多次拟合同一算法，但允许每次运行算法时，学习算法中使用的随机性发生变化。每次运行时，在相同的测试集上评估模型，并记录分数。在所有重复结束时，使用平均值和标准偏差总结分数的分布。

模型在多次运行中的平均性能给出了特定数据集上特定模型的平均性能。分数的分布或标准差给出了学习算法引入的方差的概念。
...
重复结束时，平均分数约为 84.7%，标准偏差约为 0.5%。这意味着，对于在特定训练集上训练并在特定测试集上评估的特定模型，99%的运行将导致 83.2%到 86.2%之间的测试精度，给定与平均值的三个标准偏差。
...
我们可以通过评估适合不同大小训练数据集的 MLP 模型的性能来研究这个问题。

作为一个基础，我们可以定义大量的例子，我们认为这些例子足以学习这个问题，比如 10 万个。我们可以将此作为训练示例数量的上限，并在测试集中使用这许多示例。我们将定义一个在这个数据集上表现良好的模型，作为有效地学习了两个圆问题的模型。

然后，我们可以通过用不同大小的训练数据集拟合模型来进行实验，并在测试集上评估它们的性能。

太少的例子将导致低的测试精度，可能是因为所选择的模型超过了训练集或者训练集不足以代表问题。

太多的例子会导致好的，但可能比理想的测试精度稍低，可能是因为所选的模型没有能力学习如此大的训练数据集的细微差别，或者数据集过度代表了问题。

训练数据集大小对模型测试准确性的线形图应该显示收益递减点的增长趋势，甚至可能是性能的最终轻微下降。
...
然后可以重复调用这个函数。我猜想，在 1000 到 10000 个问题的例子之间的某个地方就足以学会这个问题，其中足够意味着测试精度只有很小的分数差异。
...
在运行结束时，将创建一个线图来显示列车组大小和模型测试组精度之间的关系。我们预计会看到一个指数曲线，从低精度到收益递减点。

还创建了每个测试集大小的分数分布的方框图和触须图。随着训练集大小的增加，我们预计测试精度的分布会缩小。
...
我们可以看到测试精度从 100 个示例急剧增加到 1000 个示例，之后性能似乎趋于平稳。
...
用不同大小的训练集训练的多层线性规划在圆问题上的测试集精度的盒须图

结果表明，所选的 MLP 模型配置可以通过 1，000 个示例很好地学习问题，通过 5，000 个和 10，000 个示例可以看到相当适度的改进。也许有一个 2500 个例子的最佳点，用不到 5000 个例子就能得到 84%的测试集准确率。

随着越来越多的数据被提供给模型，神经网络的性能可以不断提高，但是模型的容量必须被调整以支持数据的增加。最终，会有一个收益递减点，在这个点上，更多的数据将无法提供更多关于如何最好地建模映射问题的洞察。对于更简单的问题，比如两个圆，这个点会比更复杂的问题更快达到，比如对象的照片分类。

这项研究强调了应用机器学习的一个基本方面，具体来说，你需要足够多的问题示例来学习未知底层映射函数的有用近似。

我们几乎从来没有丰富或过多的训练数据。因此，我们的重点通常是如何最经济地使用可用数据，以及如何避免过度拟合训练数据集中存在的统计噪声。

## 研究测试集大小与测试集准确性

给定一个固定的模型和一个固定的训练数据集，需要多少测试数据来实现模型性能的准确估计？
...
运行该示例会报告每个不同大小测试集的测试集精度，在同一数据集上训练的 10 个模型之间取平均值。
...
这令人惊讶，因为天真的预期可能是，与训练集大小相同的测试集将是模型精度的合理近似值，例如，就好像我们对 2000 个示例数据集执行了 50%/50%的分割。
...
圆问题上不同测试集尺寸的测试集精度分布的盒须图

应该指出的是，我们并没有报告所选模型在从领域中随机抽取的示例上的平均性能。模型和例子是固定的，唯一的变化来源是学习算法。

该研究证明了模型的估计精度对测试数据集的大小有多敏感。这是一个重要的考虑因素，因为通常很少考虑测试集的大小，使用 70%/30%或 80%/20%的常见分割进行训练/测试分割。
...
在实践中，我们必须与太小而无法学习未知底层映射函数的训练数据集和太小而无法合理逼近模型在问题上的性能的测试数据集作斗争。
