...
> 原文:[https://machinelearning master . com/polyak-神经网络-模型-权重-集成/](https://machinelearningmastery.com/polyak-neural-network-model-weight-ensemble/)

最后更新于 2020 年 8 月 28 日

神经网络的训练过程是一个具有挑战性的优化过程，常常无法收敛。

这可能意味着训练结束时的模型可能不是用作最终模型的稳定或性能最佳的权重集。

解决这个问题的一种方法是在训练结束时使用多个模型的平均权重。这被称为 Polyak-Ruppert 平均，并且可以通过使用模型权重的线性或指数递减加权平均值来进一步改进。除了产生更稳定的模型之外，平均模型权重的性能也可以产生更好的性能。
...
*   使用训练结束时观察到的模型的平均权重来创建模型，可以产生更稳定、有时性能更好的解决方案。
...
对来自单次训练运行的多个模型的权重进行平均具有平息噪声优化过程的效果，该噪声优化过程可能由于学习超参数(例如，学习速率)的选择或正在学习的映射函数的形状而变得有噪声。结果是最终的模型或一组权重，可能会提供更稳定，也许更准确的结果。
...
一个问题是，我们不知道有多少模型适合组合以获得良好的性能。我们可以通过用最后的 *n* 个模型评估模型加权平均集成来解决这个问题，并改变 *n* 个模型来查看有多少模型产生了良好的性能。
...
报告每个单独保存的模型的性能，以及一个集合模型，该集合模型具有从所有模型到每个模型(包括每个模型)的平均权重，从训练运行结束后向后工作。
...
还会创建一个线图，显示每个单个模型的测试精度(蓝点)和模型权重集合的性能(橙色线)。

我们可以看到，对模型权重进行平均确实会使最终模型的性能变得均衡，并且性能至少与运行的最终模型一样好。
...
单一模型测试性能(蓝点)和模型权重集合测试性能(橙色线)的线图
...
运行该示例再次报告每个单个模型的性能，这一次测试每个平均模型权重集合的准确性，其中模型的贡献呈线性下降。
...
折线图显示了性能的提升，并显示了与使用平均加权系综相比，在所创建的不同大小的系综上，测试精度方面更稳定的性能。
...
线性衰减的单一模型测试性能(蓝点)和模型权重集合测试性能(橙色线)的线图
...
具有指数衰减的单模型测试性能(蓝点)和模型权重集合测试性能(橙色线)的线图
