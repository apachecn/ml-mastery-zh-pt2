...
《深度神经网络规范化的辍学生入门》摄影:乔斯林·金霍恩，版权所有。
...
这具有模型学习训练数据中的统计噪声的效果，当在新数据(例如测试数据集)上评估模型时，这导致较差的性能。泛化误差由于过度拟合而增加。
...
这种概念化表明，可能是辍学打破了网络层共同适应纠正前几层的错误的情况，反过来使模型更加健壮。

> …单位可能会改变，以弥补其他单位的错误。这可能会导致复杂的共同适应。这反过来会导致过度拟合，因为这些协同适应不会推广到看不见的数据。[…]
...
由于丢失，网络的权重将比正常情况下更大。因此，在最终确定网络之前，首先根据所选的辍学率来调整权重。然后，网络可以正常使用来进行预测。

> 如果一个单元在训练期间以概率 p 被保留，则该单元的输出权重在测试时乘以 p

——[辍学:防止神经网络过拟合的简单方法](http://jmlr.org/papers/v15/srivastava14a.html)，2014。

相反，权重的重新调整可以在训练时执行，在小批量结束时的每次权重更新之后。这有时被称为“T0”反向下降，在训练过程中不需要对重量进行任何修改。Keras 和 PyTorch 深度学习库都以这种方式实现辍学。

> 在测试时，我们按辍学率缩小输出。[……]请注意，这个过程可以通过在训练时执行两个操作，并在测试时保持输出不变来实现，这通常是它在实践中的实现方式
...
> ……与其他标准的计算成本低廉的正则化器(如权重衰减、滤波器范数约束和稀疏活动正则化)相比，dropout 更有效。辍学也可以结合其他形式的正规化，以产生进一步的改善。

—第 265 页，[深度学习](https://amzn.to/2NJW3gE)，2016。

## 使用 drop 的示例

本节总结了一些在最近的研究论文中使用辍学的例子，为如何和在哪里使用辍学提供建议。
...
> …我们使用相同的辍学率–所有隐藏单元的辍学率为 50%，可见单元的辍学率为 20%

Nitish Srivastava 等人在 2014 年的期刊论文中介绍了名为“[from:防止神经网络过度拟合的简单方法](http://jmlr.org/papers/v15/srivastava14a.html)”的 from，该论文在广泛的计算机视觉、语音识别和文本分类任务中使用了 drop，并发现它在每个问题上的性能都得到了一致的提高。

> 我们为不同领域的数据集上的分类问题训练了脱落神经网络。我们发现，与不使用丢弃的神经网络相比，丢弃提高了所有数据集的泛化性能。

在计算机视觉问题上，不同的辍学率与最大范数权重约束一起被用于整个网络层。
...
> 我们在[模型的]前两个完全连接的层中使用了 drop。没有辍学，我们的网络表现出大量的过度匹配。退出大约是收敛所需迭代次数的两倍。
...
> ……贝叶斯优化程序了解到，对于我们训练的这种规模的 sigmoid 网来说，丢弃是没有帮助的。总的来说，ReLUs 和辍学似乎合作得很好。
...
在 LSTMs 的情况下，可能需要对输入和循环连接使用不同的辍学率。

### 辍学率

丢失超参数的默认解释是训练层中给定节点的概率，其中 1.0 表示没有丢失，0.0 表示该层没有输出。

隐藏层中脱落的良好值介于 0.5 和 0.8 之间。输入层使用较大的辍学率，例如 0.8。
...
一个很好的经验法则是将丢弃之前的层中的节点数除以建议的丢弃率，并将其用作使用丢弃的新网络中的节点数。例如，一个网络有 100 个节点，建议的辍学率为 0.5，当使用 drop 时，将需要 200 个节点(100 / 0.5)。
...
与其猜测适合你的网络的辍学率，不如系统地测试不同的辍学率。

例如，测试值在 1.0 和 0.1 之间，增量为 0.1。

这将帮助您发现什么最适合您的特定模型和数据集，以及模型对辍学率有多敏感。一个更敏感的模型可能是不稳定的，可能会受益于尺寸的增加。
...
*   第 7.12 节辍学，[深度学习](https://amzn.to/2NJW3gE)，2016。
*   第 4.4.3 节添加辍学生，[Python 深度学习](https://amzn.to/2wVqZDq)，2017。
...
*   [作为自适应正则化的辍学训练](https://arxiv.org/abs/1307.1493)，2013。
...
*   [辍学(神经网络)，维基百科](https://en.wikipedia.org/wiki/Dropout_(neural_networks))。
*   [用于视觉识别的正则化卷积神经网络](https://cs231n.github.io/neural-networks-2/#reg)
*   [“辍学生”是如何构思的？有没有“啊哈”的时刻？](https://www.reddit.com/r/MachineLearning/comments/4w6tsv/ama_we_are_the_google_brain_team_wed_love_to/d64yyas)
