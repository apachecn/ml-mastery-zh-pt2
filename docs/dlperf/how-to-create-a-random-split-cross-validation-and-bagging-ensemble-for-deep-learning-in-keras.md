...
实现模型之间差异的一种方法是在可用训练数据的不同子集上训练每个模型。通过使用重采样方法，例如交叉验证和自举，在训练数据的不同子集上自然地训练模型，该重采样方法被设计来估计模型在通常看不见的数据上的平均表现。在该估计过程中使用的模型可以被组合在所谓的基于重采样的集成中，例如交叉验证集成或自举聚合(或打包)集成。
...
此外，通常在打包中继续添加集成成员，直到集成表现稳定下来，因为打包不会过度填充数据集。我们将再次像前面的例子一样将成员人数限制在 10 人。
...
也许是由于引导抽样过程，我们看到每个模型的实际表现在更大的看不见的保持数据集上稍差一些。
