...
> 原文:[https://machinelearning master . com/how-create-random-split-cross-validation-and-bagging-ensemble-for-deep-learning-in-keras/](https://machinelearningmastery.com/how-to-create-a-random-split-cross-validation-and-bagging-ensemble-for-deep-learning-in-keras/)
...
实现模型之间差异的一种方法是在可用训练数据的不同子集上训练每个模型。通过使用重采样方法，例如交叉验证和自举，在训练数据的不同子集上自然地训练模型，该重采样方法被设计来估计模型在通常看不见的数据上的平均性能。在该估计过程中使用的模型可以被组合在所谓的基于重采样的集成中，例如交叉验证集成或自举聚合(或打包)集成。

在本教程中，您将发现如何为深度学习神经网络模型开发一套不同的基于重采样的集成。

完成本教程后，您将知道:

*   如何使用随机拆分来评估模型性能，并根据模型开发一个集成。
*   如何使用 10 倍交叉验证来评估性能，并开发交叉验证集成。
*   如何使用 bootstrap 估计性能，并使用 bagging 集成组合模型。
...
组合来自多个模型的预测可以产生更稳定的预测，并且在某些情况下，预测具有比任何贡献模型更好的性能。

有效的合奏需要意见不同的成员。每个成员都必须有技能(例如，表现比随机机会更好)，但理想情况下，在不同的方面表现良好。从技术上讲，我们可以说，我们更喜欢系综成员的预测具有低相关性，或者预测误差。
...
重新采样系综模型会对您的项目做出一些特定的假设:

*   需要对未知数据的模型性能进行稳健估计；如果没有，则可以使用单个训练/测试分割。
*   使用模型集合有可能提升性能；如果没有，则可以使用适合所有可用数据的单一模型。
*   在训练数据集的样本上拟合一个以上的神经网络模型的计算成本并不令人望而却步；如果没有，所有的资源都应该放在一个模型上。

神经网络模型非常灵活，因此由重采样集成提供的性能提升并不总是可能的，因为基于所有可用数据训练的单个模型可以表现得很好。

因此，使用重采样集合的最佳点是这样的情况，其中需要性能的稳健估计，并且可以拟合多个模型来计算该估计，但是还需要在性能估计期间创建的一个(或多个)模型用作最终模型(例如，新的最终模型不能拟合所有可用的训练数据)。
...
接下来，我们必须将数据集分成训练集和测试集。我们将使用测试集来评估模型的性能，并使用学习曲线绘制训练期间的性能。我们将使用 90%的数据进行训练，10%用于测试集。
...
我们可以尝试一种简单的重采样方法，在训练集和测试集中重复生成新的数据集随机分割，并拟合新的模型。计算模型在每个分割中的平均性能将给出模型泛化误差的更好估计。

然后，我们可以组合在随机分裂上训练的多个模型，期望集成的性能可能比平均单个模型更稳定和更好。
...
所以现在我们有 5000 个例子来训练我们的模型并估计它的一般性能。我们还有 50，000 个例子，可以用来更好地逼近单个模型或集合的真实总体性能。
...
我们不知道有多少模型会在集合中有用。很可能会有一个收益递减点，在此之后，更多成员的加入不再改变合奏的表现。

然而，我们可以评估从 1 到 10 的不同集合大小，并在看不见的保持数据集上绘制它们的性能。
...
然后，我们在未看到的数据集上评估每个模型的性能，以及从 1 到 10 个模型的模型集合的性能。
...
创建了一个图表，将未看到的保持数据集上每个单独模型的精度显示为蓝点，将给定数量的成员(从 1 到 10)的集合的性能显示为橙色线和点。
...
该图确实显示了一些单个模型可以比一组模型表现得更好(橙色线上方的蓝点)，但是我们无法选择这些模型。这里，我们证明了在没有额外数据(例如，样本外数据集)的情况下，4 到 8 个成员的集合将比随机选择的训练测试模型给出更好的平均性能。

更多的重复(例如 30 或 100)可以导致更稳定的合奏表现。

## 交叉验证一起

作为估计模型平均性能的重采样方法，重复随机分割的一个问题是它是乐观的。

一种被设计得不太乐观并因此被广泛使用的方法是 [k 倍交叉验证法](https://machinelearningmastery.com/k-fold-cross-validation/)。

该方法偏差较小，因为数据集中的每个示例在测试数据集中仅使用一次来估计模型性能，这与随机训练测试分割不同，在随机训练测试分割中，给定的示例可能被多次用于评估模型。

该过程有一个名为 k 的参数，它表示给定数据样本要分成的组数。每个模型得分的平均值提供了对模型性能的偏差较小的估计。k 的典型值是 10。

因为神经网络模型的训练在计算上非常昂贵，所以通常在交叉验证期间使用表现最好的模型作为最终模型。

或者，来自交叉验证过程的结果模型可以被组合以提供交叉验证集合，该集合可能比给定的单个模型平均具有更好的性能。
...
现在我们已经收集了在 10 个折叠上评估的 10 个模型，我们可以使用它们来创建一个交叉验证集合。在系综中使用所有 10 个模型似乎是直观的，然而，我们可以像上一节一样，从 1 到 10 个成员来评估系综的每个子集的准确性。
...
运行该示例首先打印 10 个模型在交叉验证的每个折叠上的性能。
...
与前面的示例一样，模型性能之间的真正差异在于模型精度的几分之一。

橙色线表明，随着成员数量的增加，集合的准确性会增加到收益递减的程度。

我们可以看到，至少在这种情况下，在一个集成中使用四个或更多的模型进行交叉验证比几乎所有的单个模型都具有更好的性能。
...
该方法可用于估计神经网络模型的性能。给定样本中未选择的示例可以用作测试集来估计模型的性能。

自举是一种用于估计模型性能的稳健方法。它确实受到乐观偏见的影响，但在实践中通常几乎和 k 倍交叉验证一样准确。
...
过度约束和过度训练的神经网络可以获得更好的性能。然而，在这个例子中，我们将使用前面章节中相同的 MLP。

此外，通常在打包中继续添加集成成员，直到集成性能稳定下来，因为打包不会过度填充数据集。我们将再次像前面的例子一样将成员人数限制在 10 人。
...
也许是由于引导抽样过程，我们看到每个模型的实际性能在更大的看不见的保持数据集上稍差一些。
...
我们看到，在大约四个成员之后，袋装集成在保持数据集上取得了比任何单个模型更好的性能。毫无疑问，鉴于个别车型的平均表现略低。
...
显示单个模型精度(蓝点)与不同装袋尺寸的套装精度的线图

## 扩展ˌ扩张

本节列出了一些您可能希望探索的扩展教程的想法。

*   **单车型**。将每个集合的性能与基于所有可用数据训练的一个模型进行比较。
*   **CV 集合大小**。为交叉验证集成试验更大和更小的集成大小，并比较它们的性能。
*   **装袋集合限额**。增加打包团队的成员数量，找到收益递减点。
