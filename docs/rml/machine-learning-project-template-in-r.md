...
为了解决 R 中的预测建模机器学习问题，您需要将 R 映射到这个过程中。它们可能需要稍加修改或重命名，以适应“R 做事方式”(例如caret包)。

下一节将提供这种映射，并详细说明每个任务以及您可以使用的子任务和Caret包的类型。
