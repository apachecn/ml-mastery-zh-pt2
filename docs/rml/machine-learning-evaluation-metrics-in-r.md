...
当您使用Caret评估模型时，使用的默认度量是用于分类问题的**准确性**和用于回归的 **RMSE** 。但是 caret 支持一系列其他流行的评估指标。

在下一节中，您将逐步了解Caret提供的每个评估指标。每个示例都提供了一个完整的案例研究，您可以将其复制并粘贴到项目中，以适应您的问题。
...
这些是用于评估caret中二进制和多类类别数据集算法的默认指标。
...
在这篇文章中，你发现了不同的度量标准，可以用来评估你的机器学习算法在 R 中使用caret的表现。具体来说:
