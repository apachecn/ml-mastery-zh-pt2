...
黄金分割搜索与离散有序列表的模式搜索相关，例如二分搜索法和斐波那契搜索。它与其他线性搜索算法(如布伦特法)相关，更一般地说，与其他直接搜索优化方法(如NelderMead法)相关。
