...
我们可以看到，对于具有红色、绿色和蓝色三个通道的输入图像，每个过滤器都有三个深度(这里我们使用的是通道-最后一个格式)。我们可以把一个滤镜想象成一个有三个图像的图，每个通道一个，或者把三个都压缩成一个彩色图像，或者只看第一个通道，假设其他通道看起来都一样。问题是，我们还有另外 63 个我们可能想要可视化的过滤器。
