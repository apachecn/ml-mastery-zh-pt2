...
这包括在模型的训练和评估期间缩放像素值和使用[图像数据增强](https://machinelearningmastery.com/how-to-configure-image-data-augmentation-when-training-deep-learning-neural-networks/)技术。

一个有用的捷径不是测试广泛的选项，而是考虑最先进的模型所使用的数据准备、训练时间增加和测试时间增加的类型，这些模型在具有挑战性的计算机视觉数据集上取得了显著的最佳性能，即使用 ImageNet 数据集的大规模视觉识别挑战(ILSVRC)。

在本教程中，您将发现使用卷积神经网络为图像分类任务准备和增加照片的最佳实践。

完成本教程后，您将知道:

*   图像数据可能应该通过减去在训练数据集上计算的每通道平均像素值来居中。
*   训练数据的增加可能包括随机重新缩放、水平翻转、对亮度、对比度和颜色的扰动，以及随机裁剪。
*   测试时间增加可能既包括对每个图像的多次重新缩放，也包括对图像的每个重新缩放版本的多个不同系统裁剪的预测。
...
此外，图像数据增强可用于提高模型性能并减少泛化误差，测试时间增强可用于提高拟合模型的预测性能。

与其猜测什么可能是有效的，一个好的做法是仔细看看文献中描述的性能最好的模型上使用的数据准备、训练时间增加和测试时间增加的类型。
...
描述在这个年度竞赛中赢得任务或在任务中表现良好的模型的论文可以被回顾，以便发现图像增强所执行的数据准备类型。反过来，在为您自己的图像分类任务准备图像数据时，这些可以用作建议和最佳实践。

在接下来的部分中，我们将回顾四个顶级模型中使用的数据准备和图像增强:它们是 Supervisor/AlexNet、Google net/盗梦空间、VGG 和 ResNet。
...
> 数据增强的第一种形式包括生成图像平移和水平反射。我们通过从 256×256 幅图像中随机提取 224×224 个面片(以及它们的水平反射)并在这些提取的面片上训练我们的网络来做到这一点。

——[深度卷积神经网络的 ImageNet 分类](https://papers.nips.cc/paper/4824-imagenet-classification-with-deep-convolutional-neural-networks)，2012。

第二种类型的增强是随机改变图像的亮度。

> 第二种形式的数据增强包括改变训练图像中的 RGB 通道的强度。具体来说，我们在整个 ImageNet 训练集中对 RGB 像素值集执行 PCA。对于每个训练图像，我们添加找到的主成分的倍数，其幅度与相应的[特征值](https://machinelearningmastery.com/introduction-to-eigendecomposition-eigenvalues-and-eigenvectors/)乘以随机变量成比例，该随机变量从均值为零且标准偏差为 0.1 的高斯中提取。

——[深度卷积神经网络的 ImageNet 分类](https://papers.nips.cc/paper/4824-imagenet-classification-with-deep-convolutional-neural-networks)，2012。

### 测试时间增加

执行测试时间增加是为了给拟合模型做出稳健预测的每一个机会。
...
图像数据增强是所描述的方法的组合，依赖于 AlexNet 和 VGG。
...
*   **列车时间增加**。需要增加训练时间，最常见的包括调整输入图像的大小和裁剪，以及修改图像，如移位、翻转和改变颜色。
*   **测试时间增加**。测试时间增加集中于输入图像的系统裁剪，以确保检测到输入图像中存在的特征。
...
在本教程中，您发现了使用卷积神经网络为图像分类任务准备和增加照片的最佳实践。
