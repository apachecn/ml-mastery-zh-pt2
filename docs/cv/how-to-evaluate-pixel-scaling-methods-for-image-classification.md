...
*   **精度更高**。在给定的数据准备方案下，平均模型性能明显显示出更好的准确性。
...
预计美国有线电视新闻网在这个问题上表现非常好，尽管为这个实验选择的模型不一定表现好或最好。相反，它必须是熟练的(比随机的更好)，并且必须允许不同数据准备方案的影响在学习速度和/或模型性能方面有所不同。
...
然后可以为三个数据准备方案中的每一个调用*repeat _ evaluation()*函数，并且可以报告方案下模型性能的均值和标准差。
...
不同像素缩放方法下美国有线电视新闻网在 MNIST 性能的盒须图

## 第五步。分析结果

为简洁起见，我们将只在数据准备方案的比较中查看模型性能。这项研究的扩展还将关注每个像素缩放方法下的学习速率。
