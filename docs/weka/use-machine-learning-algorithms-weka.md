...
您可以使用的专业技巧是将标准算法配置保存到文件中。单击算法配置底部的“保存”按钮。输入一个文件名，明确标注算法名称和要保存的配置类型。您可以稍后在Weka浏览器、实验者和Weka的其他地方加载算法配置。当您确定了一套标准算法配置，并希望在一个又一个问题上重用时，这是最有价值的。
