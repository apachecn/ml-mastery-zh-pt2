...
但是你必须做一些工作，一点阅读，一点在Weka的修补。你想开始应用机器学习，对吗？
...
在本课中，您将在Weka浏览器中加载一个标准数据集。

1.  启动Weka(点击鸟图标)，这将启动Weka图形用户界面选择器。
2.  点击“浏览器”按钮，这将打开Weka浏览器界面。
...
可以从[Weka数据集下载网页](https://sourceforge.net/projects/weka/files/datasets/)下载一套标准回归机器学习数据集。下载数据集-回归问题的 numeric.jar 档案，标题为:
...
*   你发现了如何启动和使用Weka探索者和Weka实验环境，也许是第一次。
