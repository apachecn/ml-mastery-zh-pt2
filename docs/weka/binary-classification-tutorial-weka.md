...
我们可以创建数据集的新副本，标记缺失的数据，然后用每个属性的平均值进行估算。这可能有助于假设属性分布平稳变化的方法，如逻辑回归和基于实例的方法。
...
只看原始分类准确率，我们可以看到数据集视图中缺失的估算值似乎导致了模型准确率的降低。与原始结果相比，标准化和规范化结果之间除了几个百分点的分数之外，似乎没有什么区别。这表明我们可能会坚持使用原始数据集。
