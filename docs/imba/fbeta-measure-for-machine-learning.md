...
Fbeta 度量是使用精度和召回率计算的。

**精度**是计算正类正确预测百分比的指标。**回忆**计算所有可能做出的积极预测中积极类的正确预测的百分比。最大化精度将最小化假阳性错误，而最大化召回将最小化假阴性错误。

**F-measure** 被计算为精确度和召回率的调和平均值，给予每个相同的权重。它允许使用单个分数同时考虑精确度和召回率来评估模型，这在描述模型的性能和比较模型时很有帮助。

**Fbeta 测度**是 F 测度的推广，增加了一个配置参数β。默认β值为 1.0，与 F-测度相同。较小的β值(如 0.5)在计算分数时更重视精度，而较少考虑召回，而较大的β值(如 2.0)在计算分数时更重视精度，而更多考虑召回。
...
*   在二元分类问题中，精度和召回率提供了两种方法来总结正类的错误。
...
在我们深入研究 Fbeta-measure 之前，我们必须回顾用于评估分类模型所做预测的精度和召回度量的基础。
...
运行该示例演示计算所有不正确和所有正确的预测类标签的精度，分别显示无精度和完美精度。
...
现在我们已经熟悉了精度和召回率，让我们回顾一下 F-measure。

## 衡量

精确度和召回率衡量正类可能出现的两种错误。

最大化精度可以最小化误报，最大化召回可以最小化误报。

F-Measure 或 F-Score 提供了一种将精度和召回率结合到一个度量中的方法，该度量可以捕获这两个属性。
...
F-measure 的直觉是，这两个度量在重要性上是平衡的，只有好的精度和好的召回率才能产生好的 F-measure。
...
这将给我们 50%的精度，因为一半的预测是假阳性。它会给我们完美的回忆，因为我们不会有假阴性。

对于我们在示例中使用的平衡数据集，一半的预测是真阳性，一半是假阳性；因此，精确率将为 0.5%或 50%。将 50%的感知精度和完美召回率结合起来，将得到一个惩罚的 F-测度，具体来说就是 50%到 100%之间的调和平均值。
...
在某些问题上，我们可能会对更注重精度的 F-测度感兴趣，例如当误报对于最小化更重要，但漏报仍然重要时。

在其他问题上，我们可能会对一个更注重回忆的 F-测度感兴趣，比如什么时候假阴性对最小化更重要，但假阳性仍然重要。

解决办法是 Fbeta-措施。

Fbeta 测度是 F 测度的抽象，其中[谐波均值](https://en.wikipedia.org/wiki/Harmonic_mean)计算中的精度和召回率的平衡由一个称为*β*的系数控制。

*   Fbeta = ((1 + beta^2) *精度*召回)/ (beta^2 *精度+召回)
...
*   **f 0.5-测量**(β= 0.5):精度权重更大，召回权重更小。
*   **F1-测量**(β= 1.0):在精度和召回率上平衡权重。
*   **F2-测量**(β= 2.0):精度权重较小，召回权重较大
...
*   F-Measure = ((1 + 1^2) *精度*召回)/ (1^2 *精度+召回)
*   F-Measure = (2 *精度*召回)/(精度+召回)

考虑我们有 50 感知精度和完美回忆的情况。对于这种情况，我们可以手动计算 F1 度量，如下所示:
...
运行该示例证实了完美的精度和 50%的召回率以及 0.667 的 F1 度量，证实了我们的计算(带舍入)。
...
如果最大化精度可以最小化假阳性，最大化召回可以最小化假阴性，那么**f 0.5-度量将更多的注意力放在最小化假阳性上**而不是最小化假阴性上。

f 0.5-测量值计算如下:

*   f 0.5-测量= ((1 + 0.5^2) *精度*召回)/ (0.5^2 *精度+召回)
*   f 0.5-测量= (1.25 *精度*召回)/ (0.25 *精度+召回)
...
我们预计β值为 0.5 会导致该场景的得分较低，因为精度得分较低，召回率很高。

这正是我们所看到的，在 F1 分数计算为 0.667 的相同情况下，F0.5 的测量值为 0.555。精度在计算中发挥了更大的作用。
...
如果最大化精度可以最小化假阳性，最大化召回可以最小化假阴性，那么 **F2-measure 比最小化假阳性更注重最小化假阴性**。

F2 测量值计算如下:

*   F2-Measure = ((1 + 2^2) *精度*召回)/ (2^2 *精度+召回)
*   F2-测量= (5 *精度*召回)/ (4 *精度+召回)
...
我们预计 2.0 的 *beta* 值将导致该场景的更高得分，因为召回具有满分，这将比精度性能差的情况得到提升。
