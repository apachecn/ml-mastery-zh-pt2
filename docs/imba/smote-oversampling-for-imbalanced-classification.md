...
正如论文中提到的，当与多数类的欠采样(如随机欠采样)结合使用时，SMOTE 的性能会更好。
...
您可以尝试测试少数族裔和多数族裔的不同比率(例如，更改 *sampling_strategy* 参数)，看看是否有可能进一步提升性能。

另一个需要探索的领域是，在创建每个新的合成示例时，测试在 SMOTE 过程中选择的 k 近邻的不同值。默认值为 *k=5* ，尽管较大或较小的值会影响创建的示例类型，进而可能影响模型的性能。
...
与边界线-SMOTE 不同，我们可以看到类重叠最多的示例具有最多的焦点。对于这些低密度示例可能是异常值的问题，ADASYN 方法可能会将过多的注意力放在特征空间的这些区域，这可能会导致更差的模型性能。
