...
原因是，许多机器学习算法被设计成对分类数据进行操作，每个类具有相等数量的观察值。当情况并非如此时，算法可以了解到，为了获得良好的性能，极少数示例并不重要，可以忽略。
...
在经过转换的数据集上评估模型，并删除或合成示例，可能会对性能提供一个误导性的、或许是乐观的估计。
...
尽管在训练数据集上单独使用过采样或欠采样方法可能是有效的，但是实验表明，将这两种类型的技术一起应用通常可以在所得的变换数据集上获得模型拟合的更好的整体性能。
...
最简单的配对包括将 SMOTE 与随机欠采样相结合，在提出该方法的论文中，该方法的性能优于单独使用 SMOTE。

通常将 SMOTE 与从数据集中选择要删除的示例的欠采样方法配对，该过程在 SMOTE 之后应用于数据集，从而允许编辑步骤同时应用于少数和多数类。其目的是从两个类中去除沿着类边界的噪声点，这似乎具有更好的分类器性能适合变换数据集的效果。
