...
分类精度是一种度量，它将分类模型的表现总结为正确预测的数量除以预测的总数。
...
用于评估分类预测模型表现的最常见指标是分类精度。通常，预测模型的准确性很好(高于 90%的准确性)，因此，根据模型的错误率来总结模型的表现也是非常常见的。
...
当类别分布稍有偏差时，准确性仍然是一个有用的指标。当类分布中的偏差严重时，精度可能成为模型表现的不可靠度量。
...
这意味着，基于平衡类分布开发的分类精度直觉将被应用，并且将是错误的，误导从业者认为模型具有良好甚至优秀的表现，而事实上，它没有。
...
然后对模型进行评估，并报告平均精度。我们可以看到，正如预期的那样，朴素分类器的表现与类分布完全匹配。
