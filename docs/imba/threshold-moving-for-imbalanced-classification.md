...
对于那些类别严重失衡的分类问题，默认阈值可能会导致性能不佳。因此，提高预测不平衡分类问题上的概率的分类器的性能的简单和直接的方法是调整用于将概率映射到类别标签的阈值。
...
与 ROC 曲线不同的是，[精度-召回率曲线](https://machinelearningmastery.com/roc-curves-and-precision-recall-curves-for-classification-in-python/)只关注正类(少数类)分类器的性能。
