...
*   **2020 年 1 月更新**:关于精确和召回目标的改进语言。修正了精度和召回寻求最小化的错别字(谢谢评论！).
...
如何计算不平衡分类的精度、召回率和 F-Measure[瓦尔德马尔合并](https://flickr.com/photos/paxx/8161204857/)图片，保留部分权利。
...
精度和召回度量是根据混淆矩阵中的单元格来定义的，特别是像真阳性和假阴性这样的术语。

现在我们已经复习了混淆矩阵，让我们更仔细地看看精度度量。
...
> 精度评估被分类为阳性的实例中正确分类实例的比例…

—第 52 页，[从不平衡数据集](https://amzn.to/307Xlva)中学习，2018。

### 二元分类的精度

在具有两个类别的不平衡分类问题中，精度计算为真阳性数除以真阳性和假阳性总数。
...
结果是精度为 0.75，这是一个合理的值，但并不突出。

你可以看到，精度仅仅是所有正面预测中正确正面预测的比率，或者少数民族预测的精度。

考虑同一个数据集，其中一个模型预测了 50 个属于少数民族的例子，其中 45 个是真阳性，5 个是假阳性。我们可以按如下方式计算该模型的精度:
...
在具有两个以上类别的不平衡分类问题中，精度计算为所有类别的真阳性之和除以所有类别的真阳性和假阳性之和。

*   精度= C 中的和 C 真阳性 _c/C 中的和 C(真阳性 _c +假阳性 _ C)

例如，我们可能有一个不平衡的多类分类问题，其中多数类是负类，但有两个正少数类:类 1 和类 2。精度可以量化两个正类中正确预测的比率。

考虑一个少数与多数类比率为 1:1:100 的数据集，即每个正类的比率为 1:1，少数类与多数类的比率为 1:100，每个少数类中有 100 个示例，多数类中有 10，000 个示例。

一个模型预测了第一个少数民族阶层的 70 个例子，其中 50 个是正确的，20 个是不正确的。它预测第二类 150 分，正确 99 分，错误 51 分。该模型的精度可以计算如下:

*   精度=(真阳性 _1 +真阳性 _2) /((真阳性 _1 +真阳性 _2) +(假阳性 _1 +假阳性 _2))
*   精度= (50 + 99) / ((50 + 99) + (20 + 51))
*   精度= 149 / (149 + 71)
*   精度= 149 / 220
*   精度= 0.677

我们可以看到，随着少数民族类别数量的增加，精度度量计算也在增加。
...
接下来，我们可以使用相同的函数以 1:1:100 计算多类问题的精度，每个少数类有 100 个示例，多数类有 10，000 个示例。一个模型预测 1 类有 50 个真阳性和 20 个假阳性，2 类有 99 个真阳性和 51 个假阳性。
...
最大化精度将最小化假阳性的数量，而最大化召回将最小化假阴性的数量。

*   **精度**:当**最大限度地减少误报**是重点时合适。
*   **回忆**:适当的时候**尽量减少假阴性**是重点。

有时候，我们想要积极阶层的优秀预测。我们想要高精度和高召回率。

这可能很有挑战性，因为召回率的提高往往是以精确度的降低为代价的。

> 在不平衡的数据集上，目标是在不损害精度的情况下提高召回率。然而，这些目标往往是相互冲突的，因为为了增加少数民族的人口比例，计划生育的数量也经常增加，导致精度降低。
...
分类精度被广泛使用，因为它是用于总结模型性能的单一度量。

F-Measure 提供了一种方法，可以将精度和召回率结合到一个能够同时获取这两种属性的度量中。

单独来看，精确和回忆都不能说明全部情况。我们可以有极好的精度和可怕的回忆，或者，可怕的精度和极好的回忆。F-measure 提供了一种用一个分数来表达两个问题的方法。

一旦为二进制或多类分类问题计算了精度和召回率，这两个分数就可以合并到 F-Measure 的计算中。
...
> ……F1-measure 对精度和召回率进行同等加权，是从不平衡数据中学习时最常使用的变量。

—第 27 页，[不平衡学习:基础、算法和应用](https://amzn.to/32K9K6d)，2013。

与精确度和召回率一样，较差的 F-Measure 分数为 0.0，最佳或完美的 F-Measure 分数为 1.0

例如，一个完美的精度和召回分数将产生一个完美的 F-Measure 分数:
...
我们可以看到，好的召回水平抵消了差的精度，给出了一个好的或合理的 F-measure 分数。
...
*   [如何计算深度学习模型的精度、召回率、F1 等](https://machinelearningmastery.com/how-to-calculate-precision-recall-f1-and-more-for-deep-learning-models/)
