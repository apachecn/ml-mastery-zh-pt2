...
仅用两个变量，我们可能会对同时发生的两个事件的概率感兴趣，称为联合概率:一个事件给定另一个事件发生的概率称为条件概率，或者仅一个事件的概率而不考虑其他变量，称为边际概率。

这些类型的概率很容易定义，但是它们背后的直觉可能需要一些时间来理解，需要一些可行的例子来修改。

在本教程中，您将发现计算联合概率、边际概率和条件概率背后的直觉。

完成本教程后，您将知道:

*   如何计算独立随机变量的联合概率、边际概率和条件概率？
*   如何从联合随机变量中收集观测值并构造联合概率表？
*   如何从联合概率表中计算联合概率、边际概率和条件概率？
...
探索联合概率和边际概率的一个好的起点是考虑独立的随机变量，因为计算非常简单。
...
此表捕获了两个随机变量 dice1 和 dice2 事件的联合概率分布。这很无聊，但我们可以用它来加深我们对自变量联合概率和边际概率的理解。
...
我们也可以用这个表来计算边际概率。这被计算为 dice1 的一整列概率或 dice2 的一行概率的总和。

例如，我们可以用 dice2 作为表格最后一行的概率总和来计算滚动 6 的边际概率。这大约是 0.166 或 16.666%，正如我们可能直观预期的那样。
...
开发一个联合概率表是一个有用的工具，有助于更好地理解如何计算和探索联合概率和边际概率。

在下一节中，让我们来看一个更复杂的带有因变量的例子。

## 两个城市的天气概率

我们可以利用两个相依随机变量的事件联合概率表来发展联合概率和边际概率的直觉。
...
因此，某市晴天的边际概率为 0.35 或 35%。

我们可以通过连续计算一个事件在部分或全部概率范围内的边际概率来为 city 做同样的事情。例如，城市中雨天的概率将被计算为沿着表格底部一行的概率之和:

*   p(市=多雨)= 0/20 + 1/20 + 3/20
*   p(市=多雨)= 4/20

因此，城市雨天的边际概率为 0.2%或 20%。

边际概率通常是有趣和有用的，更新联合概率表以包括它们是一个好主意；例如:
...
例如，如果我们只知道给定城市中晴天的条件概率和城市的边际概率，我们可以将联合概率计算为:
...
在本教程中，您发现了计算联合概率、边际概率和条件概率背后的直觉。
