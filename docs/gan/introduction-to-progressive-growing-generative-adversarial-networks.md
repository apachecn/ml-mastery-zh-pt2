...
与贪婪的逐层预训练不同，渐进式增长GAN涉及添加层块和逐步添加层块，而不是直接添加它们。
...
在本节中，我们可以回顾一下论文中描述的渐进式增长GAN所取得的一些令人印象深刻的成果。
