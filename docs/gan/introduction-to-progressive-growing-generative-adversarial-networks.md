...
*   渐进增长的GAN模型能够生成高分辨率的真实感合成人脸和对象，非常逼真。
...
此外，GAN模型的训练仍然不稳定，即使存在一套经验技术，旨在提高模型训练过程的稳定性。
...
与贪婪的逐层预训练不同，渐进生长GAN涉及添加层块和逐步添加层块，而不是直接添加它们。
...
在本节中，我们可以回顾一下论文中描述的渐进式生长GAN所取得的一些令人印象深刻的成果。
