...
测试时间增加，简称 TTA，是一种提高预测模型技能的技术。

它通常用于提高深度学习模型在图像数据集上的预测性能，其中预测在测试数据集中每个图像的多个增强版本之间进行平均。

尽管在图像数据集和神经网络模型中很受欢迎，但是测试时间增强可以与表格数据集上的任何机器学习算法一起使用，例如在回归和分类预测建模问题中经常看到的算法。

在本教程中，您将发现如何在 scikit-learn 中对表格数据使用测试时间增加。

完成本教程后，您将知道:

*   测试时间增强是一种提高模型性能的技术，通常用于图像数据集上的深度学习模型。
*   如何用 scikit-learn 在 Python 中实现回归和分类表格数据集的测试时间增加。
*   如何调整测试时间增加中使用的合成样本数量和统计噪声量？

**用我的新书[机器学习的数据准备](https://machinelearningmastery.com/data-preparation-for-machine-learning/)启动你的项目**，包括*分步教程*和所有示例的 *Python 源代码*文件。

我们开始吧。

![Test-Time Augmentation With Scikit-Learn](img/b802b00dd0b7393e8d68bef2e72912a0.png)
...
TTA 常用于图像分类，其中图像数据扩充用于创建每个图像的多个修改版本，例如裁剪、缩放、旋转和其他特定于图像的修改。因此，该技术提高了图像分类算法在标准数据集上的性能。

在他们 2015 年发表的题为“用于大规模图像识别的非常深卷积网络”的论文中，作者使用了水平翻转测试时间增强:
...
*   [如何使用测试时间增加进行更好的预测](https://machinelearningmastery.com/how-to-use-test-time-augmentation-to-improve-model-performance-for-image-classification/)

虽然经常用于图像数据，但是测试时间增加也可以用于其他数据类型，例如表格数据(例如数字的行和列)。

有许多方法可以将 TTA 用于表格数据。一种简单的方法是创建添加了小高斯噪声的数据行的副本。然后可以对来自复制行的预测进行平均，以产生用于回归或分类的改进预测。

我们将探索如何使用 scikit-learn Python 机器学习库来实现这一点。

首先，让我们定义一个评估模型的标准方法。

## 标准模型评估

在这一节中，我们将在下一节介绍测试时间增加之前，探索评估机器学习模型的典型方法。
...
运行该示例使用重复的分层 k 倍交叉验证和测试时间增加来评估逻辑回归。
...
网格搜索每次在测试时间增加期间进行预测时创建的合成示例的数量可能会很有趣。
...
运行该示例报告了在测试时间增加期间创建的不同数量的合成示例的准确性。
...
创建了示例数量与分类精度的线图，显示奇数个示例通常比偶数个示例具有更好的性能。
...
在测试时间增加期间，我们也可以用添加到测试集中的示例中的随机噪声量来执行相同的灵敏度分析。
...
运行该示例报告了在测试时间增加期间添加到创建的示例中的不同统计噪声量的准确性。
...
[SMOTE](https://machinelearningmastery.com/smote-oversampling-for-imbalanced-classification/) 是一种流行的过采样方法，用于重新平衡训练数据集中每个类别的观测值。它可以创建合成的例子，但是需要类标签的知识，这使得它不容易在测试时间增加中使用。
...
在本教程中，您发现了如何在 scikit-learn 中对表格数据使用测试时间增加。
