...
在本教程中，您将发现如何在 scikit-learn 中对表格数据使用测试时间扩充。

完成本教程后，您将知道:

*   测试时间增强是一种提高模型表现的技术，通常用于图像数据集上的深度学习模型。
*   如何用 scikit-learn 在 Python 中实现回归和分类表格数据集的测试时间扩充。
...
在本教程中，您发现了如何在 scikit-learn 中对表格数据使用测试时间扩充。
