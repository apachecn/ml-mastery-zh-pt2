...
与通过将多类分类问题划分为固定数量的二进制分类问题来提供类似解决方案的[一对一和一对一方法](https://machinelearningmastery.com/one-vs-rest-and-one-vs-one-for-multi-class-classification/)不同，纠错输出码技术允许将每个类编码为任意数量的二进制分类问题。当使用超定表示时，它允许额外的模型充当“纠错”预测，从而可以产生更好的预测性能。
