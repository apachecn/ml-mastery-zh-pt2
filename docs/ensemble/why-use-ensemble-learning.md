...
*   使用系综的一个主要好处是提高了系综中任何贡献成员的平均预测表现。
...
模型的平均精度或误差是预期表现的总结，而事实上，一些模型在不同的数据子集上表现更好，而一些模型表现更差。
...
我们明确地使用集成学习来寻求更好的预测表现，例如更低的回归误差或更高的分类精度。

> ……有一种比明智的算法选择更容易、更强大的提高模型精度的方法:可以将模型集合成集合。
