...
在本节中，我们将仔细研究一些您应该考虑为渐变增强系综进行调整的超参数，以及它们对模型表现的影响。
...
回想一下，决策树被顺序地添加到模型中，以努力纠正和改进先前的树所做的预测。因此，更多的树往往更好。树的数量也必须与学习速率相平衡，例如，更多的树可能需要更小的学习速率，更少的树可能需要更大的学习速率。
...
在这种情况下，我们将网格搜索梯度增强的四个关键超参数:集成中使用的树的数量、学习速率、用于训练每棵树的子样本大小以及每棵树的最大深度。我们将为每个超参数使用一系列流行的表现良好的值。
