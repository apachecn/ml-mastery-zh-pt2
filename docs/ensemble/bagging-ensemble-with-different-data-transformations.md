...
bagging 的思想可以推广到用于改变训练数据集和在数据的每个改变版本上拟合相同模型的其他技术。一种方法是使用改变输入变量的规模和概率分布的数据变换，作为训练类似 bagging 的集合的贡献成员的基础。我们可以称之为数据转换打包或数据转换集成。
...
*   如何开发一个用于分类的数据转换集成，并确认该集成的性能优于任何贡献成员。
...
由于没有更好的名称，我们将其称为“**数据转换打包**或“**数据转换集成**”
...
我们将使用标准实践来评估模型，在这种情况下，使用三次重复和 10 次重复的重复分层 k-fold 交叉验证。将使用所有折叠和重复的分类准确度的平均值来报告性能。
...
我们可以通过评估每个单独模型的性能并将结果与整体进行比较来检查这一点。
...
运行该示例首先报告每个单独模型的平均和标准分类精度，最后是组合模型的集成的性能。
...
接下来，我们可以通过拟合和评估我们打算在集成中使用的基础模型来建立合成数据集的性能基线，在本例中，基础模型是[决策树回归器](https://scikit-learn.org/stable/modules/generated/sklearn.tree.DecisionTreeRegressor.html)。

该模型将使用重复的 k-fold 交叉验证进行评估，重复 3 次，重复 10 次。数据集上的模型性能将使用平均绝对误差(MAE)报告。scikit-learn 将反转分数(使其为负数)，以便框架可以最大化分数。因此，我们可以忽略分数上的符号。
...
和以前一样，我们的期望是，合奏会提升任何单个模型的性能。如果没有，那么应该选择表现最好的个人模型。
...
运行该示例首先报告每个单独模型的 MAE，最后是组合模型的集合的性能。

**注**:考虑到算法或评估程序的随机性，或数值精度的差异，您的[结果可能会有所不同](https://machinelearningmastery.com/different-results-each-time-in-machine-learning/)。考虑运行该示例几次，并比较平均结果。

我们可以看到，每个模型的表现都差不多，MAE 误差分数在 140 左右，都高于孤立使用的决策树。有趣的是，该合奏表现最好，超过了所有单个成员和没有变换的树，达到了大约 126.487 的 MAE。
