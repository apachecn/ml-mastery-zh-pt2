...
> 与装袋和随机森林不同，如果(树木数量)太大，助推会过拟合，尽管这种过拟合往往发生得很慢。
...
这就是所有现代助推算法的实现方式，这是一个有趣而重要的想法。然而，校正预测误差可能被认为是实现增强的实现细节(一个大而重要的细节)，而不是提升集成方法的本质。
...
我们可以考虑三大类助推方法；它们是:AdaBoost、经典梯度提升和现代梯度提升。
