...
*   弱学习器是比随机猜测表现稍好的模型。
*   强学习器是具有任意好的准确性的模型。
*   弱学习器和强学习器是来自计算学习理论的工具，并且为集成方法的助推类的发展提供基础。
...
> 对于二元分类，众所周知，对弱学习器的确切要求是要优于随机猜测。[……]请注意，要求基础学习器优于随机猜测对于多类问题来说太弱了，然而要求优于 50%的准确率太严格了。
...
> 一个较弱的可学习性模型，称为弱可学习性，放弃了学习器能够达到任意高精度的要求；弱学习算法只需要输出一个比随机猜测表现稍好的假设(通过逆多项式)。
...
一般来说，增强集成的目标是为预测性学习问题培养大量弱学习器，然后最好地组合它们，以实现强学习器。这是一个很好的目标，因为弱学习器容易准备但不可取，而强学习器很难准备并且非常可取。
