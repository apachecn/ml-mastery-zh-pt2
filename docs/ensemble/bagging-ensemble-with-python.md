...
> 原文:[https://machinelearning master . com/bagging-ensemble-with-python/](https://machinelearningmastery.com/bagging-ensemble-with-python/)
...
*   如何用 scikit-learn 使用 Bagging 集成进行分类和回归。
...
打包并不总是能带来改善。对于已经表现良好的低方差模型，打包会导致模型性能下降。

> 实验和理论证据都表明，打包可以将一个好的但不稳定的过程推向最优化的重要一步。另一方面，它会稍微降低稳定程序的性能。
...
scikit-learn Python 机器学习库为机器学习提供了 Bagging 集成的实现。
...
现在我们已经熟悉了使用 scikit-learn API 来评估和使用 Bagging 集成，让我们来看看如何配置模型。

## 装袋超参数

在本节中，我们将仔细研究一些您应该考虑为 Bagging 集成进行调整的超参数，以及它们对模型性能的影响。

### 探索树的数量

Bagging 算法的一个重要超参数是集成中使用的决策树的数量。

通常，树的数量会增加，直到模型性能稳定下来。直觉可能暗示更多的树会导致过度拟合，尽管事实并非如此。给定学习算法的随机性质，装袋和决策树算法的相关集成(如[随机森林](https://machinelearningmastery.com/implement-random-forest-scratch-python/))似乎在某种程度上不会过度拟合训练数据集。
...
默认情况下，将创建一个与原始数据集具有相同数量示例的引导样本。使用较小的数据集可以增加生成的决策树的方差，并可能导致更好的整体性能。
...
在这种情况下，结果表明，随着样本大小的增加，性能通常会提高，这突出表明默认的 100%的训练数据集大小是合理的。
...
我们可以测试不同的 k 值，以找到模型方差的正确平衡，从而作为一个袋装集成获得良好的性能。
...
我们看到，在开始时，随着样本量的增加，准确度总体呈上升趋势，随后，随着集合中使用的单个 KNN 模型的方差随着更大的 *k* 值而增加，性能略有下降。
...
为了提高方法的性能，对 bagging 算法进行了许多修改和扩展。
...
我们看到了一个总体趋势，即随着特征数量的增加，精度增加到大约 10 到 13 个特征，在这一水平上，精度大致保持不变，然后性能呈适度下降趋势。
...
> ……众所周知，Bagging 应该用于不稳定的学习者，一般来说，越不稳定，性能提升越大。

—第 52 页，[集合方法](https://amzn.to/2XZzrjG)，2012。

**问:应该用多少个文工团成员？**

模型的性能将随着决策树数量的增加而收敛到一个点，然后保持水平。

> Bagging 的性能随着集成规模(即基础学习者的数量)的增长而收敛…

—第 52 页，[集合方法](https://amzn.to/2XZzrjG)，2012。

因此，不断增加树的数量，直到数据集的性能稳定下来。
...
*   [硬化。一起。bagginger return API](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.BaggingRegressor.html)。
