...
> 在实践中，对于给定的预测问题和数据集，通常不可能先验地知道哪个学习者表现最好。
...
这就是所谓的“*超级学习者*”集成算法背后的直觉。

超级学习者算法包括首先预定义数据的 k 倍分割，然后评估同一分割数据上的所有不同算法和算法配置。然后，所有超出范围的预测都被保留下来，并用于训练一种算法，学习如何最好地组合预测。

> 这些算法可能在所使用的协变量子集、基函数、损失函数、搜索算法和调谐参数范围等方面有所不同。

——[预测中的超级学习者](https://biostats.bepress.com/ucbbiostat/paper266/)，2010。

这个模型的结果应该不会比 k 倍交叉验证期间评估的最佳表现模型差，并且有可能比任何单一模型表现更好。

超级学习者算法是由来自伯克利的马克·范德莱恩、T2、埃里克·波利和 T4 在他们 2007 年的论文《超级学习者》中提出的它发表在一份生物学杂志上，该杂志可能不受更广泛的机器学习社区的影响。
...
我们可以把“*超级学习者*”想象成一种专门针对 k-fold 交叉验证的堆叠的具体配置。
...
接下来，我们将使用 k 倍交叉验证进行超倍预测，这些预测将用作训练元模型或“T0”超级学习者的数据集。
...
既然我们已经看到了如何为回归开发一个超级学习者，让我们看一个分类的例子。
...
运行该示例首先报告每个基本模型的 RMSE(得分-m)，然后报告超级学习者本身的 RMSE。

考虑到在后端使用多线程允许使用机器的所有内核，拟合和评估非常快。

**注**:考虑到算法或评估程序的随机性，或数值精度的差异，您的[结果可能会有所不同](https://machinelearningmastery.com/different-results-each-time-in-machine-learning/)。考虑运行该示例几次，并比较平均结果。

在这种情况下，我们可以看到超级学习者表现良好。
...
*   [超级学习者:超级学习者预测，CRAN](https://cran.r-project.org/web/packages/SuperLearner/) 。
